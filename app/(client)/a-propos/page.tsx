...
export default function AboutPage() {
  ...
  useEffect(() => {
    if (brand) {
      getPage(brand.id, 'a-propos').then(data => {
        setPage(data);
        setLoading(false);
      });
    }
  ...
  return (
    <div className="bg-brand-bg pt-32 pb-20 px-6 max-w-md mx-auto">
      ...
      <h1 className="font-serif text-3xl mb-6">{page?.title || "L'Atelier"}</h1>
      
      <div className="prose prose-stone prose-sm">
        {/* Simple rendering for now, can be upgraded to Markdown/HTML parser if needed */}
